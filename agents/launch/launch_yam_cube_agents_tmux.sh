...
usage() {
  cat <<'USAGE'
Launch or preview the neutral bimanual YAM cube Codex agent fleet.

Default behavior is --dry-run: generate per-agent prompts/runners and print the
tmux commands without starting agents.

Usage:
  agents/launch/launch_yam_cube_agents_tmux.sh [--dry-run|--launch] [--attach] [--session NAME]
...
Examples:
  agents/launch/launch_yam_cube_agents_tmux.sh --dry-run
  agents/launch/launch_yam_cube_agents_tmux.sh --launch
  agents/launch/launch_yam_cube_agents_tmux.sh --launch --attach
USAGE
}

action="dry-run"
attach=0
session="${SESSION:-yam-cube-agents}"
...
prompt_template="${PROMPT_TEMPLATE:-$root/agents/launch/bimanual_yam_cube_agent_prompt.md}"
prompt_dir="${PROMPT_DIR:-$root/agents/launch/generated}"
runner_dir="$prompt_dir/runners"
log_dir="${LOG_DIR:-$root/agents/logs}"
...
require_dir "$worktree_root"
mkdir -p "$prompt_dir" "$runner_dir" "$log_dir"
