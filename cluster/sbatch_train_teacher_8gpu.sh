...
PY
    ...
    if [ '$TASK' = 'Dextrah-Cube-Grasp' ]; then
      ...
      if [ -n '$TRAJECTORY_TRACKING_TEACHER_FORCE_ENABLED' ]; then
        TASK_OVERRIDES+=(
          env.trajectory_tracking_teacher_force_enabled='$TRAJECTORY_TRACKING_TEACHER_FORCE_ENABLED'
        )
      fi
      if [ -n '$TRAJECTORY_TRACKING_TEACHER_FORCE_ALPHA_START' ]; then
        TASK_OVERRIDES+=(
          env.trajectory_tracking_teacher_force_alpha_start='$TRAJECTORY_TRACKING_TEACHER_FORCE_ALPHA_START'
        )
      fi
      if [ -n '$TRAJECTORY_TRACKING_TEACHER_FORCE_ALPHA_END' ]; then
        TASK_OVERRIDES+=(
          env.trajectory_tracking_teacher_force_alpha_end='$TRAJECTORY_TRACKING_TEACHER_FORCE_ALPHA_END'
        )
      fi
      if [ -n '$TRAJECTORY_TRACKING_TEACHER_FORCE_PHASE_END' ]; then
        TASK_OVERRIDES+=(
          env.trajectory_tracking_teacher_force_phase_end='$TRAJECTORY_TRACKING_TEACHER_FORCE_PHASE_END'
        )
      fi
      if [ -n '$TRAJECTORY_TRACKING_TEACHER_FORCE_ANNEAL_STEPS' ]; then
        TASK_OVERRIDES+=(
          env.trajectory_tracking_teacher_force_anneal_steps='$TRAJECTORY_TRACKING_TEACHER_FORCE_ANNEAL_STEPS'
        )
      fi
      if [ -n '$TRAJECTORY_TRACKING_ACTION_ALIGNMENT_COMPARE_RAW_POLICY' ]; then
        TASK_OVERRIDES+=(
          env.trajectory_tracking_action_alignment_compare_raw_policy='$TRAJECTORY_TRACKING_ACTION_ALIGNMENT_COMPARE_RAW_POLICY'
        )
      fi
    else
      TASK_OVERRIDES=(
