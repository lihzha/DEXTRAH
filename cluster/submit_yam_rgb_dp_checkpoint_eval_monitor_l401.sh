  ...
  if [ "$step" -ge "$next_threshold" ] && [ -s "$CHECKPOINT_HOST" ]; then
    ...
    job_id="$(
      sbatch --parsable \
        ...
        --export=ALL,CODE_NFS="$CODE_NFS",RESULTS_NFS="$RESULTS_NFS",CODE_COMMIT="$CODE_COMMIT",RUN_NAME="$run_name",CHECKPOINT="$snapshot",NUM_EPISODES="$NUM_EPISODES",NUM_STEPS="$NUM_STEPS",VIDEO_LENGTH="$VIDEO_LENGTH",ACTION_CHUNK_STEPS="$ACTION_CHUNK_STEPS",DEBUG_OBS_INTERVAL="$DEBUG_OBS_INTERVAL",DEBUG_OBS_MAX_FRAMES="$DEBUG_OBS_MAX_FRAMES",RENDERING_MODE="$RENDERING_MODE",CAPTURE_VIDEO="$CAPTURE_VIDEO",YAM_DEFAULT_FINGER_QPOS="$YAM_DEFAULT_FINGER_QPOS",YAM_POLICY_OBJECT_ASSET_MANIFEST_PATH="$YAM_POLICY_OBJECT_ASSET_MANIFEST_PATH",YAM_POLICY_MAX_OBJECTS="$YAM_POLICY_MAX_OBJECTS",YAM_POLICY_OBJECT_VALIDATE_USD_BOUNDS="$YAM_POLICY_OBJECT_VALIDATE_USD_BOUNDS",YAM_POLICY_TABLE_TEXTURE_DIR="$YAM_POLICY_TABLE_TEXTURE_DIR" \
        "$EVAL_WRAPPER"
    )"
    evals=$((evals + 1))
