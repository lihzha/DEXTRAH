...
SEED="${SEED:-42}"
POLICY_SAMPLE_SEED="${POLICY_SAMPLE_SEED:-42}"
BIN_DROP_CONTAINMENT_TOLERANCE_M="${BIN_DROP_CONTAINMENT_TOLERANCE_M:-0.001}"
...
payload = {
    ...
    "seed": int("$SEED"),
    "policy_sample_seed": None if "$POLICY_SAMPLE_SEED" == "" else int("$POLICY_SAMPLE_SEED"),
    "bin_drop_containment_tolerance_m": float("$BIN_DROP_CONTAINMENT_TOLERANCE_M"),
    "disable_failure_terminations": "$DISABLE_FAILURE_TERMINATIONS" == "True",
    "disable_success_termination": "$DISABLE_SUCCESS_TERMINATION" == "True",
    "stop_on_done": "$STOP_ON_DONE" == "True",
    "stop_on_bin_drop_success": "$STOP_ON_BIN_DROP_SUCCESS" == "True",
    "rendering_mode": "$RENDERING_MODE",
    "dual_camera_video": "$DUAL_CAMERA_VIDEO" == "True",
    "dual_camera_video_fps": float("$DUAL_CAMERA_VIDEO_FPS"),
  ...
  if [ "$step" -ge "$next_threshold" ] && [ -s "$CHECKPOINT_HOST" ]; then
    ...
    if ! job_id="$(
      retry_external sbatch --parsable \
        ...
        --export=ALL,CODE_NFS="$CODE_NFS",RESULTS_NFS="$RESULTS_NFS",CODE_COMMIT="$CODE_COMMIT",RUN_NAME="$run_name",CHECKPOINT="$snapshot",NUM_EPISODES="$NUM_EPISODES",NUM_STEPS="$NUM_STEPS",VIDEO_LENGTH="$VIDEO_LENGTH",ACTION_CHUNK_STEPS="$ACTION_CHUNK_STEPS",SEED="$SEED",POLICY_SAMPLE_SEED="$POLICY_SAMPLE_SEED",BIN_DROP_CONTAINMENT_TOLERANCE_M="$BIN_DROP_CONTAINMENT_TOLERANCE_M",DISABLE_FAILURE_TERMINATIONS="$DISABLE_FAILURE_TERMINATIONS",DISABLE_SUCCESS_TERMINATION="$DISABLE_SUCCESS_TERMINATION",STOP_ON_DONE="$STOP_ON_DONE",STOP_ON_BIN_DROP_SUCCESS="$STOP_ON_BIN_DROP_SUCCESS",DEBUG_OBS_INTERVAL="$DEBUG_OBS_INTERVAL",DEBUG_OBS_MAX_FRAMES="$DEBUG_OBS_MAX_FRAMES",RENDERING_MODE="$RENDERING_MODE",CAPTURE_VIDEO="$CAPTURE_VIDEO",DUAL_CAMERA_VIDEO="$DUAL_CAMERA_VIDEO",DUAL_CAMERA_VIDEO_FPS="$DUAL_CAMERA_VIDEO_FPS",YAM_DEFAULT_FINGER_QPOS="$YAM_DEFAULT_FINGER_QPOS",YAM_POLICY_OBJECT_ASSET_MANIFEST_PATH="$YAM_POLICY_OBJECT_ASSET_MANIFEST_PATH",YAM_POLICY_MAX_OBJECTS="$YAM_POLICY_MAX_OBJECTS",YAM_POLICY_OBJECT_VALIDATE_USD_BOUNDS="$YAM_POLICY_OBJECT_VALIDATE_USD_BOUNDS",YAM_POLICY_TABLE_TEXTURE_DIR="$YAM_POLICY_TABLE_TEXTURE_DIR",YAM_POLICY_GROUND_TEXTURE_DIR="$YAM_POLICY_GROUND_TEXTURE_DIR",YAM_POLICY_DOME_LIGHT_TEXTURE_DIR="$YAM_POLICY_DOME_LIGHT_TEXTURE_DIR" \
        "$EVAL_WRAPPER"
    )"; then
