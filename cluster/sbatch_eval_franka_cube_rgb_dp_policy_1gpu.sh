...
RESET_CUBE_X="${RESET_CUBE_X:-}"
RESET_CUBE_Y="${RESET_CUBE_Y:-}"
RESET_CUBE_Z="${RESET_CUBE_Z:-}"
...
REPLAY_DATASET_ACTIONS="${REPLAY_DATASET_ACTIONS:-}"
REPLAY_DATASET_EPISODE="${REPLAY_DATASET_EPISODE:-0}"
REPLAY_DATASET_START_STEP="${REPLAY_DATASET_START_STEP:-0}"
...
REPLAY_DATASET_ACTIONS_HOST="$REPLAY_DATASET_ACTIONS"
REPLAY_DATASET_ACTIONS_ARG="$REPLAY_DATASET_ACTIONS"
if [ -n "$REPLAY_DATASET_ACTIONS" ]; then
  REPLAY_DATASET_ACTIONS_HOST="$(host_path_from_container "$REPLAY_DATASET_ACTIONS")"
  REPLAY_DATASET_ACTIONS_ARG="$(container_path_from_host "$REPLAY_DATASET_ACTIONS")"
fi
...
if [ -n "$REPLAY_DATASET_ACTIONS" ] && [ ! -f "$REPLAY_DATASET_ACTIONS_HOST" ]; then
  echo "Missing replay dataset actions: $REPLAY_DATASET_ACTIONS_HOST"
  exit 2
fi
...
export RESET_CUBE_X RESET_CUBE_Y RESET_CUBE_Z
export APPEND_PHASE_PROGRESS PHASE_PROGRESS_DATASET_ARG PHASE_PROGRESS_EPISODE PHASE_PROGRESS_START_STEP
export REPLAY_DATASET_ACTIONS_ARG REPLAY_DATASET_EPISODE REPLAY_DATASET_START_STEP
...
if [ -n "$RESET_CUBE_X" ] || [ -n "$RESET_CUBE_Y" ] || [ -n "$RESET_CUBE_Z" ]; then
  echo "RESET_CUBE_X=$RESET_CUBE_X RESET_CUBE_Y=$RESET_CUBE_Y RESET_CUBE_Z=$RESET_CUBE_Z"
fi
...
if [ -n "$REPLAY_DATASET_ACTIONS" ]; then
  echo "REPLAY_DATASET_ACTIONS_ARG=$REPLAY_DATASET_ACTIONS_ARG"
  echo "REPLAY_DATASET_ACTIONS_HOST=$REPLAY_DATASET_ACTIONS_HOST"
  echo "REPLAY_DATASET_EPISODE=$REPLAY_DATASET_EPISODE REPLAY_DATASET_START_STEP=$REPLAY_DATASET_START_STEP"
fi
...
srun \
  ...
  bash -lc '
    ...
    REPLAY_ARGS=()
    if [ -n "$REPLAY_DATASET_ACTIONS_ARG" ]; then
      REPLAY_ARGS=(
        --replay_dataset_actions "$REPLAY_DATASET_ACTIONS_ARG"
        --replay_dataset_episode "$REPLAY_DATASET_EPISODE"
        --replay_dataset_start_step "$REPLAY_DATASET_START_STEP"
      )
    fi
    RESET_CUBE_ARGS=()
    if [ -n "$RESET_CUBE_X" ] || [ -n "$RESET_CUBE_Y" ] || [ -n "$RESET_CUBE_Z" ]; then
      if [ -z "$RESET_CUBE_X" ] || [ -z "$RESET_CUBE_Y" ]; then
        echo "RESET_CUBE_X and RESET_CUBE_Y must both be set for explicit cube reset"
        exit 2
      fi
      RESET_CUBE_ARGS=(--reset_cube_xy "$RESET_CUBE_X" "$RESET_CUBE_Y")
      if [ -n "$RESET_CUBE_Z" ]; then
        RESET_CUBE_ARGS+=(--reset_cube_z "$RESET_CUBE_Z")
      fi
    fi
