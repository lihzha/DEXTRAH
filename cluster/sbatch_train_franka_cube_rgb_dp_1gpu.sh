...
INIT_CHECKPOINT="${INIT_CHECKPOINT:?Set INIT_CHECKPOINT to the source RGB DP checkpoint.}"
...
CREATE_MODEL_ONLY_BOOTSTRAP="${CREATE_MODEL_ONLY_BOOTSTRAP:-True}"
COPY_FINAL_CHECKPOINT="${COPY_FINAL_CHECKPOINT:-True}"

ROBOT_STATE_DIM="${ROBOT_STATE_DIM:-12}"
APPEND_PHASE_PROGRESS="${APPEND_PHASE_PROGRESS:-true}"
...
RESUME="${RESUME:-true}"
DISTILL_MASK_MODE="${DISTILL_MASK_MODE:-normal_reset}"
DISTILL_MASK_TOLERANCE="${DISTILL_MASK_TOLERANCE:-1.0e-6}"
DISTILL_LOSS_WEIGHT="${DISTILL_LOSS_WEIGHT:-2.0}"
...
if [ ! -f "$NORMALIZER_CHECKPOINT_HOST" ]; then
  echo "Missing normalizer checkpoint: $NORMALIZER_CHECKPOINT_HOST"
  exit 2
fi
if [ ! -f "$DISTILL_REFERENCE_CHECKPOINT_HOST" ]; then
...
PY
    ...
    TRAIN_ARGS=(
      ...
      "policy.distill_reference_checkpoint=$DISTILL_REFERENCE_CHECKPOINT_ARG"
      "policy.distill_loss_weight=$DISTILL_LOSS_WEIGHT"
      "policy.distill_use_action_loss_weights=$DISTILL_USE_ACTION_LOSS_WEIGHTS"
      "task.dataset.normalizer_checkpoint=$NORMALIZER_CHECKPOINT_ARG"
