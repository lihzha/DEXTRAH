...
CONFIG_NAME="${CONFIG_NAME:-franka_cube_rgb_dp}"
...
export CREATE_MODEL_ONLY_BOOTSTRAP CONFIG_NAME ACTION_DIM ROBOT_STATE_DIM IMAGE_CHANNELS IMAGE_HEIGHT IMAGE_WIDTH
export APPEND_PHASE_PROGRESS VAL_RATIO
...
export DISTILL_LOSS_WEIGHT DISTILL_USE_ACTION_LOSS_WEIGHTS ACTION_LOSS_WEIGHTS NUM_INFERENCE_STEPS
...
echo "ACTION_DIM=$ACTION_DIM ROBOT_STATE_DIM=$ROBOT_STATE_DIM IMAGE_SHAPE=[$IMAGE_CHANNELS,$IMAGE_HEIGHT,$IMAGE_WIDTH]"
...
echo "ACTION_LOSS_WEIGHTS=${ACTION_LOSS_WEIGHTS:-<config-default>}"
...
PY
    ...
    TRAIN_ARGS=(
      ...
      "task.dataset.action_dim=$ACTION_DIM"
      "shape_meta.obs.robot_state.shape=[$ROBOT_STATE_DIM]"
      "shape_meta.obs.image.shape=[$IMAGE_CHANNELS,$IMAGE_HEIGHT,$IMAGE_WIDTH]"
    ...
    if [ -n "$ACTION_LOSS_WEIGHTS" ]; then
      TRAIN_ARGS+=("policy.action_loss_weights=$ACTION_LOSS_WEIGHTS")
    fi
