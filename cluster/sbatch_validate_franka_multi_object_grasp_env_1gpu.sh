...
export TASK RUN_NAME NUM_ENVS NUM_STEPS VIDEO_LENGTH CAPTURE_VIDEO RENDER_CHECK RENDER_CHECK_FRAMES PRINT_INTERVAL SEED
...
srun \
  ...
  bash -lc '
    ...
    case "$RENDER_CHECK" in
      True|true|1|yes|Yes)
        RENDER_ARGS=(--render_check --render_check_frames "$RENDER_CHECK_FRAMES")
        ;;
    esac
    ASSET_ARGS=()
