...
#SBATCH --job-name=dextrah_kuka_eval
#SBATCH --partition=batch_singlenode,grizzly,polar,polar3,polar4,interactive_singlenode
#SBATCH --time=0-01:00:00
#SBATCH --mem=160G
#SBATCH --cpus-per-task=16
#SBATCH --output=/lustre/fsw/portfolios/nvr/users/lzha/slurm_logs/dextrah/eval_kuka_allegro_%j.out
...
RUN_NAME="${RUN_NAME:-kuka_allegro_eval_${SLURM_JOB_ID_SAFE}_$(date +%Y%m%d_%H%M%S)}"
NUM_ENVS="${NUM_ENVS:-1}"
NUM_STEPS="${NUM_STEPS:-600}"
VIDEO_LENGTH="${VIDEO_LENGTH:-600}"
VIDEO_NAME_PREFIX="${VIDEO_NAME_PREFIX:-kuka-allegro-eval}"
PRINT_INTERVAL="${PRINT_INTERVAL:-20}"
CAPTURE_VIDEO="${CAPTURE_VIDEO:-True}"
USE_CUDA_GRAPH="${USE_CUDA_GRAPH:-False}"
SEED="${SEED:-42}"
ENABLE_ADR="${ENABLE_ADR:-False}"
OBJECTS_DIR="${OBJECTS_DIR:-visdex_objects}"
CAMERA_EYE_X="${CAMERA_EYE_X:--1.15}"
CAMERA_EYE_Y="${CAMERA_EYE_Y:--0.80}"
CAMERA_EYE_Z="${CAMERA_EYE_Z:-1.10}"
CAMERA_TARGET_X="${CAMERA_TARGET_X:--0.55}"
CAMERA_TARGET_Y="${CAMERA_TARGET_Y:-0.10}"
CAMERA_TARGET_Z="${CAMERA_TARGET_Z:-0.50}"
CHECKPOINT="${CHECKPOINT:-/results/logs/rl_games/dextrah_lstm/teacher_short_20260609_100021/nn/last_dextrah_lstm_ep_20000_rew_673.2542.pth}"
...
LOG_FILE="$NFS_ROOT/slurm_logs/dextrah/eval_kuka_allegro_${SLURM_JOB_ID_SAFE}.out"
...
export TASK RUN_NAME NUM_ENVS NUM_STEPS VIDEO_LENGTH VIDEO_NAME_PREFIX PRINT_INTERVAL CAPTURE_VIDEO USE_CUDA_GRAPH SEED
export ENABLE_ADR OBJECTS_DIR
...
echo "Running DextrAH Kuka Allegro checkpoint evaluation"
