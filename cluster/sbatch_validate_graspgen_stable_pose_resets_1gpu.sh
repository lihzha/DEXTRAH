...
STABLE_POSE_RANK_OVERRIDES="${STABLE_POSE_RANK_OVERRIDES:-}"
...
OBJECT_XY_OFFSETS_CM="${OBJECT_XY_OFFSETS_CM:-}"
SETTLE_STEPS="${SETTLE_STEPS:-240}"
SETTLED_REPLAY_STEPS="${SETTLED_REPLAY_STEPS:-0}"
SEED="${SEED:-42}"
...
export STABLE_POSE_COUNT ROLLOUT_POSE_COUNT STABLE_POSE_RANK_OVERRIDES
export STABLE_POSE_MESH_MODE STABLE_POSE_SIGMA STABLE_POSE_SAMPLES STABLE_POSE_THRESHOLD
export OBJECT_XY_OFFSETS_CM
export SETTLE_STEPS SETTLED_REPLAY_STEPS SEED TABLE_CLEARANCE MAX_ROOT_XY_DRIFT MAX_CENTER_XY_DRIFT
...
echo "ROLLOUT_POSE_COUNT=$ROLLOUT_POSE_COUNT"
echo "STABLE_POSE_RANK_OVERRIDES=$STABLE_POSE_RANK_OVERRIDES"
...
echo "OBJECT_XY_OFFSETS_CM=$OBJECT_XY_OFFSETS_CM"
echo "SETTLE_STEPS=$SETTLE_STEPS"
echo "SETTLED_REPLAY_STEPS=$SETTLED_REPLAY_STEPS"
echo "SEED=$SEED"
...
PY
    ...
    ROLLOUT_ARGS=()
    if [ -n "$ROLLOUT_POSE_COUNT" ]; then
      ROLLOUT_ARGS=(--rollout_pose_count "$ROLLOUT_POSE_COUNT")
    fi
    RANK_OVERRIDE_ARGS=()
    if [ -n "$STABLE_POSE_RANK_OVERRIDES" ]; then
      RANK_OVERRIDE_ARGS=(--stable_pose_rank_overrides "$STABLE_POSE_RANK_OVERRIDES")
    fi
    OBJECT_XY_ARGS=()
    if [ -n "$OBJECT_XY_OFFSETS_CM" ]; then
      OBJECT_XY_ARGS=(--object_xy_offsets_cm "$OBJECT_XY_OFFSETS_CM")
    fi
    ...
    VALIDATE_ARGS=(
      ...
      --settled_replay_steps "$SETTLED_REPLAY_STEPS"
      --seed "$SEED"
