...
for shard_index in $(seq 0 "$((SHARD_COUNT - 1))"); do
  ...
  job_id="$(
    sbatch --parsable \
      --job-name="$job_name" \
      --export=ALL,CODE_NFS="$CODE_NFS",RESULTS_NFS="$RESULTS_NFS",BATCH_NAME="$BATCH_NAME",BATCH_DIR="$BATCH_DIR",TOTAL_TARGET="$TOTAL_TARGET",SHARD_COUNT="$SHARD_COUNT",SHARD_INDEX="$shard_index",SHARD_TARGET="$shard_target",START_SEED="$START_SEED" \
      "$WRAPPER"
  )"
