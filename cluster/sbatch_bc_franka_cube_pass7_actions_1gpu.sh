...
TRACK_EXACT_DURING_LIFT="${TRACK_EXACT_DURING_LIFT:-True}"
TRACK_EXACT_DURING_HOLD="${TRACK_EXACT_DURING_HOLD:-True}"
...
export APPROACH_STEPS CLOSE_STEPS LIFT_STEPS HOLD_STEPS CLOSE_WIDTH LIFT_ACTION_Z HOLD_ACTION_Z
export ORACLE_GAIN ORACLE_MAX_POSITION_ACTION TRACK_ORIENTATION TRACK_EXACT_DURING_LIFT TRACK_EXACT_DURING_HOLD
...
echo "TRACK_EXACT_DURING_LIFT=$TRACK_EXACT_DURING_LIFT"
echo "TRACK_EXACT_DURING_HOLD=$TRACK_EXACT_DURING_HOLD"
...
srun \
  ...
  bash -lc '
    ...
    case "$TRACK_EXACT_DURING_LIFT" in
      False|false|0|no|No) BC_ARGS+=(--no-track_exact_during_lift) ;;
      *) BC_ARGS+=(--track_exact_during_lift) ;;
    esac
    case "$TRACK_EXACT_DURING_HOLD" in
      False|false|0|no|No) BC_ARGS+=(--no-track_exact_during_hold) ;;
      *) BC_ARGS+=(--track_exact_during_hold) ;;
    esac
