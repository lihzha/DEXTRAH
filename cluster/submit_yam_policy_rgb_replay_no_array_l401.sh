...
YAM_POLICY_BACKGROUND_TEXTURE_DIR="${YAM_POLICY_BACKGROUND_TEXTURE_DIR:-$CODE_NFS/dextrah_lab/assets/textures/floor_polyhaven}"
YAM_POLICY_BACKGROUND_TEXTURE_TILING_RANGE="${YAM_POLICY_BACKGROUND_TEXTURE_TILING_RANGE:-2.0 5.0}"
YAM_POLICY_GROUND_TEXTURE="${YAM_POLICY_GROUND_TEXTURE:-True}"
...
payload = {
    ...
    "ground_texture_enabled": "$YAM_POLICY_GROUND_TEXTURE".lower() in {"true", "1", "yes"},
    ...
    "yam_gripper_stiffness_scale": float("$YAM_GRIPPER_STIFFNESS_SCALE"),
    "yam_gripper_damping_scale": float("$YAM_GRIPPER_DAMPING_SCALE"),
    "yam_gripper_effort_scale": float("$YAM_GRIPPER_EFFORT_SCALE"),
...
for shard_index in $(seq 0 "$((SHARD_COUNT - 1))"); do
  ...
  job_id="$(
    sbatch --parsable \
      --job-name="$job_name" \
      --export=ALL,CODE_NFS="$CODE_NFS",RESULTS_NFS="$RESULTS_NFS",ACCEPTED_JSONL="$ACCEPTED_JSONL",RGB_BATCH_NAME="$RGB_BATCH_NAME",RGB_BATCH_DIR="$RGB_BATCH_DIR",SHARD_COUNT="$SHARD_COUNT",SHARD_INDEX="$shard_index",MAX_ROWS="$MAX_ROWS",RENDER_WIDTH="$RENDER_WIDTH",RENDER_HEIGHT="$RENDER_HEIGHT",RENDERING_MODE="$RENDERING_MODE",RECORD_RGB_WIDTH="$RECORD_RGB_WIDTH",RECORD_RGB_HEIGHT="$RECORD_RGB_HEIGHT",RECORD_RGB_INTERVAL="$RECORD_RGB_INTERVAL",YAM_POLICY_TABLE_TEXTURE_DIR="$YAM_POLICY_TABLE_TEXTURE_DIR",YAM_POLICY_TABLE_TEXTURE_TILING_RANGE="$YAM_POLICY_TABLE_TEXTURE_TILING_RANGE",YAM_POLICY_BACKGROUND_TEXTURE_DIR="$YAM_POLICY_BACKGROUND_TEXTURE_DIR",YAM_POLICY_BACKGROUND_TEXTURE_TILING_RANGE="$YAM_POLICY_BACKGROUND_TEXTURE_TILING_RANGE",YAM_POLICY_GROUND_TEXTURE="$YAM_POLICY_GROUND_TEXTURE",YAM_POLICY_GROUND_TEXTURE_SIZE="$YAM_POLICY_GROUND_TEXTURE_SIZE",YAM_POLICY_DOME_LIGHT_TEXTURE_DIR="$YAM_POLICY_DOME_LIGHT_TEXTURE_DIR",YAM_GRIPPER_STIFFNESS_SCALE="$YAM_GRIPPER_STIFFNESS_SCALE",YAM_GRIPPER_DAMPING_SCALE="$YAM_GRIPPER_DAMPING_SCALE",YAM_GRIPPER_EFFORT_SCALE="$YAM_GRIPPER_EFFORT_SCALE",CAMERA_EYE="$CAMERA_EYE",CAMERA_TARGET="$CAMERA_TARGET",CODE_COMMIT="$CODE_COMMIT" \
      "$WRAPPER"
  )"
