...
GRIPPER_SAMPLE_AGGREGATION="${GRIPPER_SAMPLE_AGGREGATION:-mean}"
GRIPPER_CLOSE_THRESHOLD="${GRIPPER_CLOSE_THRESHOLD:-0.5}"
GRIPPER_VOTE_THRESHOLD="${GRIPPER_VOTE_THRESHOLD:-0.5}"
...
PHASE_GRIPPER_GUARD="${PHASE_GRIPPER_GUARD:-disabled}"
ACTION_CORRECTION_MODE="${ACTION_CORRECTION_MODE:-disabled}"
ACTION_CORRECTION_BLEND="${ACTION_CORRECTION_BLEND:-1.0}"
...
DEMO_RESET_REPLICATE_ENV0_JOINT_BLEND="${DEMO_RESET_REPLICATE_ENV0_JOINT_BLEND:-False}"
...
export TASK RUN_NAME NUM_ENVS NUM_STEPS NUM_INFERENCE_STEPS NUM_ACTION_SAMPLES ACTION_CHUNK_STEPS CLIP_ACTIONS SUCCESS_WINDOW
export GRIPPER_SAMPLE_AGGREGATION GRIPPER_CLOSE_THRESHOLD GRIPPER_VOTE_THRESHOLD
...
export PHASE_GRIPPER_GUARD
export ACTION_CORRECTION_MODE ACTION_CORRECTION_BLEND
...
export DEMO_RESET_REPLICATE_ENV0_JOINT_BLEND
...
echo "GRIPPER_SAMPLE_AGGREGATION=$GRIPPER_SAMPLE_AGGREGATION"
echo "GRIPPER_CLOSE_THRESHOLD=$GRIPPER_CLOSE_THRESHOLD"
echo "GRIPPER_VOTE_THRESHOLD=$GRIPPER_VOTE_THRESHOLD"
...
if [ -n "$PHASE_PROGRESS_DATASET" ]; then
  ...
  echo "PHASE_GRIPPER_GUARD=$PHASE_GRIPPER_GUARD"
fi
echo "ACTION_CORRECTION_MODE=$ACTION_CORRECTION_MODE"
echo "ACTION_CORRECTION_BLEND=$ACTION_CORRECTION_BLEND"
if [ -n "$DEMO_RESET_DATASET" ]; then
  ...
  echo "DEMO_RESET_REPLICATE_ENV0_JOINT_BLEND=$DEMO_RESET_REPLICATE_ENV0_JOINT_BLEND"
fi
  ...
  bash -lc '
    ...
    if [ -n "$PHASE_PROGRESS_DATASET_ARG" ]; then
      PHASE_PROGRESS_ARGS=(
        ...
        --phase_gripper_guard "$PHASE_GRIPPER_GUARD"
      )
    fi
    ACTION_CORRECTION_ARGS=(
      --action_correction_mode "$ACTION_CORRECTION_MODE"
      --action_correction_blend "$ACTION_CORRECTION_BLEND"
    )
    DEMO_RESET_ARGS=()
    if [ -n "$DEMO_RESET_DATASET_ARG" ]; then
      ...
      if [ "$DEMO_RESET_REPLICATE_ENV0_JOINT_BLEND" = "True" ] || [ "$DEMO_RESET_REPLICATE_ENV0_JOINT_BLEND" = "true" ] || [ "$DEMO_RESET_REPLICATE_ENV0_JOINT_BLEND" = "1" ]; then
        DEMO_RESET_ARGS+=(--demo_reset_replicate_env0_joint_blend)
      fi
    fi
