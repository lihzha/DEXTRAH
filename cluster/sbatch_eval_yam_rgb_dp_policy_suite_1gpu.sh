...
POLICY_IMAGE_ASPECT_MODE="${POLICY_IMAGE_ASPECT_MODE:-center_crop}"
POLICY_ROBOT_STATE_DIM="${POLICY_ROBOT_STATE_DIM:-8}"
POLICY_ACTION_DIM="${POLICY_ACTION_DIM:-8}"
POLICY_APPEND_PHASE_PROGRESS="${POLICY_APPEND_PHASE_PROGRESS:-False}"
POLICY_PHASE_PROGRESS_DATASET_HOST="${POLICY_PHASE_PROGRESS_DATASET_HOST:-}"
...
if [ -n "$POLICY_PHASE_PROGRESS_DATASET_HOST" ] && [ ! -f "$POLICY_PHASE_PROGRESS_DATASET_HOST" ]; then
  echo "Missing phase/progress dataset: $POLICY_PHASE_PROGRESS_DATASET_HOST" >&2
  exit 2
fi
...
export CAPTURE_INTERVAL FPS RENDER_WIDTH RENDER_HEIGHT RECORD_RGB_WIDTH RECORD_RGB_HEIGHT RECORD_RGB_INTERVAL
export POLICY_IMAGE_HEIGHT POLICY_IMAGE_WIDTH POLICY_IMAGE_ASPECT_MODE POLICY_ROBOT_STATE_DIM POLICY_ACTION_DIM
export POLICY_APPEND_PHASE_PROGRESS POLICY_PHASE_PROGRESS_DATASET_HOST NUM_INFERENCE_STEPS NUM_ACTION_SAMPLES POLICY_SAMPLE_SEED
...
echo "POLICY_ROBOT_STATE_DIM=$POLICY_ROBOT_STATE_DIM"
echo "POLICY_ACTION_DIM=$POLICY_ACTION_DIM"
echo "POLICY_APPEND_PHASE_PROGRESS=$POLICY_APPEND_PHASE_PROGRESS"
echo "POLICY_PHASE_PROGRESS_DATASET_HOST=${POLICY_PHASE_PROGRESS_DATASET_HOST:-}"
...
echo "POLICY_IMAGE_ASPECT_MODE=$POLICY_IMAGE_ASPECT_MODE"
...
srun \
  ...
  bash -lc '
    ...
    run_policy_case() {
      ...
      local phase_progress_args=()
      if [ "$POLICY_APPEND_PHASE_PROGRESS" = "True" ] || [ "$POLICY_APPEND_PHASE_PROGRESS" = "true" ] || [ "$POLICY_APPEND_PHASE_PROGRESS" = "1" ]; then
        phase_progress_args=(--policy_append_phase_progress)
        if [ -n "$POLICY_PHASE_PROGRESS_DATASET_HOST" ]; then
          phase_progress_args+=(--policy_phase_progress_dataset "$(container_path_arg "$POLICY_PHASE_PROGRESS_DATASET_HOST")")
        fi
      fi
      /isaac-sim/python.sh /code/dextrah_lab/rl_games/render_tabletop_clutter_settle_video.py \
        --task "$TASK" \
        ...
        --policy_image_aspect_mode "$POLICY_IMAGE_ASPECT_MODE" \
        --policy_robot_state_dim "$POLICY_ROBOT_STATE_DIM" \
