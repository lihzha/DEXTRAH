...
export TASK RUN_NAME NUM_ENVS NUM_STEPS VIDEO_LENGTH VIDEO_NAME_PREFIX PRINT_INTERVAL CAPTURE_VIDEO DETERMINISTIC ACTION_SOURCE REFERENCE_MIX_ALPHA HOLD_PHASE_START HOLD_TRIGGER_LIFT_HEIGHT HOLD_CONTACT_MAX_FINGER_DIST HOLD_LIFT_HEIGHT HOLD_TARGET_POLICY HOLD_GRIPPER_ACTION SUPPRESS_SUCCESS_TERMINATION USE_CUDA_GRAPH SEED CUBE_SPAWN_XY_RANDOMIZATION TRAJECTORY_TRACKING_REFERENCE_PATH
...
srun \
  ...
  bash -lc '
    ...
    if [ -n "$TRAJECTORY_TRACKING_ACTION_ALIGNMENT_WEIGHT" ]; then
      TASK_OVERRIDES+=(
        env.trajectory_tracking_action_alignment_weight="$TRAJECTORY_TRACKING_ACTION_ALIGNMENT_WEIGHT"
      )
    fi
    if [ -n "$TRAJECTORY_TRACKING_ACTION_ALIGNMENT_PHASE_START" ]; then
      TASK_OVERRIDES+=(
        env.trajectory_tracking_action_alignment_phase_start="$TRAJECTORY_TRACKING_ACTION_ALIGNMENT_PHASE_START"
      )
    fi
    if [ -n "$TRAJECTORY_TRACKING_ACTION_ALIGNMENT_SHARPNESS" ]; then
      TASK_OVERRIDES+=(
        env.trajectory_tracking_action_alignment_sharpness="$TRAJECTORY_TRACKING_ACTION_ALIGNMENT_SHARPNESS"
      )
    fi
    if [ -n "$TRAJECTORY_TRACKING_ACTION_ALIGNMENT_USE_CONTACT_GATE" ]; then
      TASK_OVERRIDES+=(
        env.trajectory_tracking_action_alignment_use_contact_gate="$TRAJECTORY_TRACKING_ACTION_ALIGNMENT_USE_CONTACT_GATE"
      )
    fi
