...
#SBATCH --job-name=dextrah_cube_label_sweep
#SBATCH --partition=batch
#SBATCH --time=0-00:55:00
#SBATCH --mem=128G
#SBATCH --cpus-per-task=16
#SBATCH --output=/lustre/fsw/portfolios/nvr/users/lzha/slurm_logs/dextrah/sweep_franka_cube_bc_label_%j.out
...
RUN_NAME="${RUN_NAME:-franka_cube_label_recipe_sweep_${SLURM_JOB_ID_SAFE}_$(date +%Y%m%d_%H%M%S)}"
NUM_ENVS="${NUM_ENVS:-1}"
...
SUCCESS_LIFT_HEIGHT="${SUCCESS_LIFT_HEIGHT:-0.01}"
...
LOG_FILE="$NFS_ROOT/slurm_logs/dextrah/sweep_franka_cube_bc_label_${SLURM_JOB_ID_SAFE}.out"
...
export GRASP_PRIOR_LIBRARY_ARG SUCCESS_LIFT_HEIGHT ORACLE_GAIN ORACLE_MAX_POSITION_ACTION TRACK_ORIENTATION
export RENDER RENDER_RESETS RENDER_INTERVAL RECIPES
...
echo "Running DextrAH Franka cube corrective label recipe sweep"
...
echo "SUCCESS_LIFT_HEIGHT=$SUCCESS_LIFT_HEIGHT"
...
srun \
  ...
  bash -lc '
    ...
    SWEEP_ARGS=(
      sweep_franka_cube_bc_label_recipes.py
      --headless
      --device cuda:0
      --task "$TASK"
      --num_envs "$NUM_ENVS"
    ...
    if [ "$RENDER" = "1" ] || [ "$RENDER" = "true" ] || [ "$RENDER" = "True" ]; then
      ...
      SWEEP_ARGS+=(--no-render)
    fi
    if [ "$TRACK_ORIENTATION" = "1" ] || [ "$TRACK_ORIENTATION" = "true" ] || [ "$TRACK_ORIENTATION" = "True" ]; then
      SWEEP_ARGS+=(--track_orientation)
    else
      SWEEP_ARGS+=(--no-track_orientation)
    fi
    if [ -n "$RECIPES" ]; then
      IFS=";" read -r -a RECIPE_ITEMS <<<"$RECIPES"
      for item in "${RECIPE_ITEMS[@]}"; do
        if [ -n "$item" ]; then
          SWEEP_ARGS+=(--recipe "$item")
        fi
      done
    fi

    printf "label_recipe_sweep_command="
    printf "%q " /isaac-sim/python.sh "${SWEEP_ARGS[@]}"
    printf "\n"
    /isaac-sim/python.sh "${SWEEP_ARGS[@]}"
  '
...
if [ -f "$LOG_FILE" ] && grep -E "Traceback|RuntimeError:|Error executing job with overrides|ChildFailedError|Could not execute <function load" "$LOG_FILE" >/dev/null; then
  echo "Detected label recipe sweep error patterns in $LOG_FILE."
  exit 1
fi

if [ ! -s "$RUN_DIR_HOST/metrics.json" ]; then
  echo "Missing label recipe sweep metrics JSON: $RUN_DIR_HOST/metrics.json"
  exit 1
fi

echo "Label recipe sweep Done"
