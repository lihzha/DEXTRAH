...
DEBUG_OBS_MAX_FRAMES="${DEBUG_OBS_MAX_FRAMES:-0}"
...
if [ "$DEBUG_OBS_INTERVAL" -gt 0 ] && [ "$DEBUG_OBS_MAX_FRAMES" -le 0 ]; then
  DEBUG_OBS_MAX_FRAMES=$(( (NUM_STEPS + DEBUG_OBS_INTERVAL - 1) / DEBUG_OBS_INTERVAL + 2 ))
fi
...
exec env \
  ...
  EXACT_VISUAL_RESAMPLE="$EXACT_VISUAL_RESAMPLE" \
  ...
  STOP_ON_DONE=False \
  STOP_ON_BIN_DROP_SUCCESS="$STOP_ON_BIN_DROP_SUCCESS" \
