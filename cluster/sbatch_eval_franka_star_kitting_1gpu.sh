...
#SBATCH --job-name=dextrah_franka_star_eval
#SBATCH --partition=batch_singlenode,grizzly,polar,polar3,polar4,interactive_singlenode
#SBATCH --time=0-01:00:00
#SBATCH --mem=160G
#SBATCH --cpus-per-task=16
#SBATCH --output=/lustre/fsw/portfolios/nvr/users/lzha/slurm_logs/dextrah/eval_franka_star_%j.out
...
RUN_NAME="${RUN_NAME:-franka_star_eval_${SLURM_JOB_ID_SAFE}_$(date +%Y%m%d_%H%M%S)}"
NUM_ENVS="${NUM_ENVS:-1}"
NUM_STEPS="${NUM_STEPS:-600}"
VIDEO_LENGTH="${VIDEO_LENGTH:-600}"
VIDEO_NAME_PREFIX="${VIDEO_NAME_PREFIX:-franka-star-kitting-eval}"
...
CAMERA_TARGET_Y="${CAMERA_TARGET_Y:-0.05}"
CAMERA_TARGET_Z="${CAMERA_TARGET_Z:-0.77}"
CHECKPOINT="${CHECKPOINT:?Set CHECKPOINT to a checkpoint path visible inside the container, e.g. /results/logs/rl_games/dextrah_franka_star_kitting/<run>/nn/foo.pth}"
...
LOG_FILE="$NFS_ROOT/slurm_logs/dextrah/eval_franka_star_${SLURM_JOB_ID_SAFE}.out"
...
export TASK RUN_NAME NUM_ENVS NUM_STEPS VIDEO_LENGTH VIDEO_NAME_PREFIX PRINT_INTERVAL CAPTURE_VIDEO DETERMINISTIC USE_CUDA_GRAPH SEED
...
echo "Running DextrAH Franka star-kitting checkpoint evaluation"
