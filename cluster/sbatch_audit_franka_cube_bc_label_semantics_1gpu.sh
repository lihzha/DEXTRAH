...
#SBATCH --job-name=dextrah_cube_bc_label
#SBATCH --partition=batch
#SBATCH --time=0-00:45:00
#SBATCH --mem=128G
#SBATCH --cpus-per-task=16
#SBATCH --output=/lustre/fsw/portfolios/nvr/users/lzha/slurm_logs/dextrah/audit_franka_cube_bc_label_%j.out
...
RUN_NAME="${RUN_NAME:-franka_cube_bc_label_semantics_${SLURM_JOB_ID_SAFE}_$(date +%Y%m%d_%H%M%S)}"
NUM_ENVS="${NUM_ENVS:-1}"
NUM_RESETS="${NUM_RESETS:-2}"
SEED="${SEED:-42}"
...
POLICY_LABEL="${POLICY_LABEL:-bc_policy}"
...
LOG_FILE="$NFS_ROOT/slurm_logs/dextrah/audit_franka_cube_bc_label_${SLURM_JOB_ID_SAFE}.out"
...
POLICY_CHECKPOINT_ARG=""
if [ -n "$POLICY_CHECKPOINT" ]; then
  POLICY_CHECKPOINT_HOST="$(container_to_host_path "$POLICY_CHECKPOINT")"
  POLICY_CHECKPOINT_ARG="$(host_to_container_path "$POLICY_CHECKPOINT")"
  if [ ! -f "$POLICY_CHECKPOINT_HOST" ]; then
    echo "Missing policy checkpoint: $POLICY_CHECKPOINT_HOST"
    exit 2
  fi
fi
...
export GRASP_PRIOR_LIBRARY_ARG POLICY_CHECKPOINT_ARG POLICY_LABEL
export APPROACH_STEPS CLOSE_STEPS LIFT_STEPS CLOSE_WIDTH LIFT_ACTION_Z ORACLE_GAIN ORACLE_MAX_POSITION_ACTION
export TRACK_ORIENTATION RENDER RENDER_RESETS RENDER_INTERVAL
...
echo "Running DextrAH Franka cube BC label semantics audit"
...
echo "POLICY_CHECKPOINT_ARG=$POLICY_CHECKPOINT_ARG"
...
srun \
  ...
  bash -lc '
    ...
    AUDIT_ARGS=(
      audit_franka_cube_bc_label_semantics.py
      --headless
      --device cuda:0
      --task "$TASK"
      --num_envs "$NUM_ENVS"
    ...
    if [ -n "$POLICY_CHECKPOINT_ARG" ]; then
      AUDIT_ARGS+=(--policy_checkpoint "$POLICY_CHECKPOINT_ARG")
    fi
    if [ "$RENDER" = "1" ] || [ "$RENDER" = "true" ] || [ "$RENDER" = "True" ]; then
      AUDIT_ARGS+=(--render)
    else
      AUDIT_ARGS+=(--no-render)
    fi
    if [ "$TRACK_ORIENTATION" = "1" ] || [ "$TRACK_ORIENTATION" = "true" ] || [ "$TRACK_ORIENTATION" = "True" ]; then
      AUDIT_ARGS+=(--track_orientation)
    else
      AUDIT_ARGS+=(--no-track_orientation)
    fi

    printf "bc_label_audit_command="
...
if [ -f "$LOG_FILE" ] && grep -E "Traceback|RuntimeError:|Error executing job with overrides|ChildFailedError|Could not execute <function load" "$LOG_FILE" >/dev/null; then
  echo "Detected BC label semantics audit error patterns in $LOG_FILE."
  exit 1
fi

if [ ! -s "$RUN_DIR_HOST/metrics.json" ]; then
  echo "Missing BC label semantics audit metrics JSON: $RUN_DIR_HOST/metrics.json"
  exit 1
fi

echo "BC label semantics audit Done"
