...
def _add_origin(parent: ET.Element, attrs: dict[str, str]) -> None:
    ...
    rpy = _quat_wxyz_to_rpy(quat)  # MJCF and USD use wxyz; URDF origin uses rpy.
...
def _download_assets(repo_id: str, force: bool) -> Path:
    ...
    source_xml = yam_mjcf_dir / "bimanual_yam_linear_flattened.xml"
    ...
    if assets_root.exists() and force:
        for path in (assets_root / "yam_mujoco", assets_root / "yam_urdf", assets_root / "yam_usd"):
            if path.exists():
                shutil.rmtree(path)
...
def _resolved_geom_type(geom: ET.Element) -> str:
    ...
    if "mesh" in geom.attrib:
        return "mesh"
    if geom.attrib.get("class", "").endswith("_collision"):
        return "capsule"
    return "sphere"
...
def _add_visual_or_collision(link: ET.Element, geom: ET.Element, mesh_files: dict[str, str]) -> None:
    ...
    if geom_class.endswith("_collision") or "collider" in geom.attrib.get("name", ""):
        collision = ET.SubElement(
            link,
            "collision",
            {"name": geom.attrib.get("name", f"{link.attrib['name']}_collision")},
        )
...
def _add_joint(urdf_root: ET.Element, parent_name: str, body: ET.Element) -> None:
    ...
    if mjcf_joint is None:
        joint = ET.SubElement(
            urdf_root,
            "joint",
            {"name": f"{child_name}_fixed", "type": "fixed"},
        )
    else:
    ...
    ET.SubElement(
        ...
        "limit",
        {
            "lower": f"{lower:.9g}",
            "upper": f"{upper:.9g}",
            "effort": f"{_joint_effort(mjcf_joint.attrib['name']):.9g}",
            "velocity": "5.0",
        },
    )
...
def main() -> int:
    ...
    urdf_path = _generate_urdf_from_mjcf(source_xml, args.force)
    ...
    try:
        output_path = _convert_urdf(urdf_path, args.force)
        print(output_path)
    finally:
