"""Combine gated Franka cube contact-relabel NPZs.

Each input must be an accepted relabel set produced by
``make_contact_relabel_set_report.py``. The script concatenates only the rows
already present in those accepted NPZs, rebuilds cumulative episode ends, and
preserves per-episode rollout/reset metadata for later train/eval audits.
"""
...
def main() -> None:
    ...
    for path in input_paths:
        ...
        obs = _read_required(data, "obs", path).astype(np.float32)
        ...
        if obs.ndim != 2 or obs.shape[1] != 21:
            raise ValueError(f"{path}: expected obs shape (N,21), got {obs.shape}")
        if action.shape != (obs.shape[0], 7):
            raise ValueError(f"{path}: expected action shape ({obs.shape[0]},7), got {action.shape}")
        if phase_ids.shape != (obs.shape[0],):
            raise ValueError(f"{path}: expected phase_ids shape ({obs.shape[0]},), got {phase_ids.shape}")
        if episode_ends.ndim != 1 or episode_ends.size == 0 or int(episode_ends[-1]) != int(obs.shape[0]):
            raise ValueError(f"{path}: episode_ends must be cumulative exclusive ends ending at obs length")
        ...
        if "rollout_reset_joint_blend_alpha" in data.files:
            values = np.asarray(data["rollout_reset_joint_blend_alpha"], dtype=np.float32).reshape(-1)
            if values.shape[0] != n_episodes:
                raise ValueError(
                    f"{path}: rollout_reset_joint_blend_alpha has {values.shape[0]} rows "
                    f"for {n_episodes} episodes"
                )
            joint_alphas.extend(float(v) for v in values.tolist())
        else:
            joint_alphas.extend(float("nan") for _ in range(n_episodes))

        if "rollout_reset_cube_pos_blend_alpha" in data.files:
            values = np.asarray(data["rollout_reset_cube_pos_blend_alpha"], dtype=np.float32).reshape(-1)
            if values.shape[0] != n_episodes:
                raise ValueError(
                    f"{path}: rollout_reset_cube_pos_blend_alpha has {values.shape[0]} rows "
                    f"for {n_episodes} episodes"
                )
            cube_alphas.extend(float(v) for v in values.tolist())
        else:
            cube_alphas.extend(float("nan") for _ in range(n_episodes))
    ...
    summary = {
        ...
        "episode_ends": episode_ends_out.astype(int).tolist(),
    ...
    (report_path.parent / "combined_contact_relabel_summary.json").write_text(
        json.dumps(_to_builtin(summary), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )

    lines = [
        "# Combined Contact Relabel Dataset",
        "",
        f"- output: `{output_path}`",
        f"- input count: `{len(input_paths)}`",
        f"- obs/action: `{tuple(obs_out.shape)}` / `{tuple(action_out.shape)}`",
        f"- episodes: `{int(episode_ends_out.shape[0])}`",
        f"- episode ends: `{episode_ends_out.astype(int).tolist()}`",
    ...
    for row in source_rows:
        lines.append(
            ...
            f"`{row['obs_shape'][0]}` | `{row['rollout_ids']}` |"
        )
    report_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print("FRANKA_CUBE_COMBINED_CONTACT_RELABEL " + json.dumps(summary, sort_keys=True))
